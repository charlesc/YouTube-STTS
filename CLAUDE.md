...
- 需要 `mlx-whisper`（僅限 Apple Silicon）以進行本地端聲音轉錄，只有在**沒有字幕、需要轉錄**時才會被 import（延遲載入，`utils/video_processor.py:transcribe_audio_with_whisper`）——模型倉庫 `WHISPER_MODEL_REPO`（預設 `mlx-community/whisper-large-v3-mlx`）會在第一次使用時下載並快取。
...
2. `yt-dlp` 將影片下載到 `work_dir/video.mp4`，並依 `config.SUBTITLE_LANGS`（預設 `['zh-TW', 'zh-Hant', 'en', 'ko', 'ja']`，此為優先順序）嘗試抓字幕檔 `work_dir/video.<lang>.vtt`。
3. **字幕優先策略**：若上述任一語言的字幕檔存在，就直接使用（`subtitle_used=True`），不進行聲音轉錄。否則透過 `ffmpeg` 擷取聲音，再以 `mlx_whisper.transcribe(...)` 進行本地端轉錄，並整理成與字幕相同的 VTT 格式。
