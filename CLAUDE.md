...
**即時進度回報**：`process_video()`、`transcribe_audio()`、`process_vtt()`、`translate_batch()`、`summarize()` 都接受可選的 `on_progress(message: str)` callback，在每個階段開始時呼叫一次（下載中、找到字幕、轉錄中、翻譯第 X/Y 批、分段摘要第 X/Y 段、擷取截圖等）。`main.py:_run_video_processing()` 把這個 callback 接到 `_jobs[job_id]['progress']`，前端輪詢時顯示這個欄位，不再是整個處理過程只停在「正在處理視頻...」不會變化。所有接受 `on_progress` 的函式都把它設成可選參數（`on_progress or _noop_progress`），沒傳的呼叫端（例如獨立測試、`__main__` 區塊）行為不變。
...
- **`'apple'`（預設）**：透過 `subprocess` 呼叫 `apple_llm_bridge/`（獨立的 Swift Package）。介面是一行 JSON stdin → 一行 JSON stdout，見 `apple_llm_bridge/Sources/apple-llm-bridge/main.swift` 開頭註解，支援兩種 `mode`：
  ...
  - **摘要巢狀結構**：原本的摘要 prompt 只說「用 `<ul><li>` 格式」，沒有規範巢狀結構，模型會把「主題/重點/結論/總結」四個部分全部攤平成同一層的 `<li>`（「3-5個關鍵要點」那句話本身也變成一個空的標題項）；分段摘要合併時更明顯——模型常常把每段落各自的 3-5 點原封不動串接，變成十幾二十點的落落長清單，而不是真的重新統整。現在 `_summary_prompt`／`_merge_summary_prompt`（`utils/vtt_translator.py`，兩者分開設計，因為合併步驟需要的是「統整、去重複」而不是單純摘要一段文字）改用固定的 `<h4>` 分節模板，明確禁止巢狀清單、禁止新增未列出的章節。即使如此，實測地端小模型仍不一定會遵守「重點最多 5 點」的指示，`summarize()` 最後一定會套用 `_cap_bullet_points()`——不管模型輸出幾點，程式碼強制只保留前 `_SUMMARY_MAX_BULLET_POINTS`（5）個 `<li>`，不依賴模型自律。`_partial_summary_prompt` 也從「條列 3-5 點」改成「一段 2-3 句話的段落」，避免合併步驟收到一堆現成的 `<li>` 就直接照抄。
